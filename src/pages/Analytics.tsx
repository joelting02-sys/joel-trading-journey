...
import { Activity, AlertTriangle, BarChart3, CalendarDays, CircleDollarSign, Gauge, Target, TrendingDown } from "lucide-react";
...
const money = (value: number, currency: string) => formatSignedCurrencyConverted(value, currency as CurrencyCode, 0);
const pct = (value: number) => `${value.toFixed(2)}%`;

type DirectionRow = { name: string; row: { count: number; pnl: number; winRate: number }; color: string };
...
export default function Analytics() {
  ...
  const directionRows: DirectionRow[] = [
    { name: "Long", row: directions.long, color: GREEN },
    { name: "Short", row: directions.short, color: RED },
  ];
  ...
  return <Layout title={labels.title}>
    ...
    <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-5">
      ...
      ].map(([label, value, color]) => <div key={String(label)} className="rounded-xl border border-border/70 bg-bg-surface px-4 py-4"><div className="text-[10px] font-semibold uppercase tracking-[.11em] text-text-muted">{label}</div><div className={`tj-number mt-2 text-xl font-semibold ${String(color)}`}>{value}</div></div>)}
    </div>
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      ...
      <Panel title={labels.direction} icon={Target}><div className="grid grid-cols-[180px_1fr] items-center gap-4 px-5 py-5"><div className="h-[180px]"><Doughnut data={directionData} options={{ responsive: true, maintainAspectRatio: false, cutout: "72%", plugins: { legend: { display: false } } } as any} /></div><div className="space-y-3">{directionRows.map(({ name, row, color }) => <div key={name} className="border-b border-border-subtle pb-3 last:border-0"><div className="flex items-center justify-between text-xs text-text-secondary"><span>{language === "zh" ? (name === "Long" ? "做多" : "做空") : name}</span><span className="tj-number">{row.count} trades</span></div><div className="tj-number mt-1 text-base font-semibold" style={{ color }}>{money(row.pnl, currency)}</div><div className="text-[11px] text-text-muted">{row.winRate}% win rate</div></div>)}</div></div></Panel>
